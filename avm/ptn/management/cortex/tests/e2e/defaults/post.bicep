...
module myTestVNet 'br/public:avm/res/network/virtual-network:0.1.6' = {
  name: '${customRpName}-vnet'
  params: {
    addressPrefixes: [crpNetworkTestSpok1.properties.addressPrefix]
    name: '${customRpName}-vnet'
    tags: {
      network: crpNetworkTestSpok1.properties.addressPrefix
    }
    subnets: [
      {
        addressPrefix: cidrSubnet(crpNetworkTestSpok1.properties.addressPrefix, 25, 0)
        name: 'FrontendSubnet'
      }
      {
        addressPrefix: cidrSubnet(crpNetworkTestSpok1.properties.addressPrefix, 25, 1)
        delegations: [
          {
            name: 'netappDel'
            properties: {
...
output mySubnetIP string = crpNetworkTestSpok1.properties.subnetIP
output myCidr int = crpNetworkTestSpok1.properties.cidr
output networkTestSpok1 object = crpNetworkTestSpok1
